...
abstract contract BaseUniswapAdapter is FlashLoanReceiverBase, IBaseUniswapAdapter, Ownable {
  ...
  constructor(
    ...
    address wethAddress
  ) public FlashLoanReceiverBase(addressProvider) {
    ORACLE = IPriceOracleGetter(addressProvider.getPriceOracle());
  ...
  function _swapExactTokensForTokens(
    ...
    uint256 expectedMinAmountOut =
      amountToSwap.mul(fromAssetPrice.mul(10**toAssetDecimals)).div(toAssetPrice.mul(10**fromAssetDecimals)).percentMul(
  ...
  function _swapTokensForExactTokens(
    ...
    uint256[] memory amounts =
      UNISWAP_ROUTER.swapTokensForExactTokens(amountToReceive, maxAmountToSwap, path, address(this), block.timestamp);
