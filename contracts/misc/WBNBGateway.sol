...
contract WBNBGateway is IWBNBGateway, Ownable {
  ...
  /**
   * @dev Sets the WBNB address and the AddressProvider address. Infinite approves lending pool.
   * @param wbnb Address of the Wrapped Ether contract
   **/
  ...
  /**
   * @dev deposits WBNB into the reserve, using native BNB. A corresponding amount of the overlying asset (aTokens)
   * is minted.
  ...
  function withdrawBNB(
    ...
    IAToken aWBNB = IAToken(ILendingPool(lendingPool).getReserveData(address(WBNB)).aTokenAddress);
  ...
  function repayBNB(
    ...
    (uint256 stableDebt, uint256 variableDebt) =
      Helpers.getUserCurrentDebtMemory(onBehalfOf, ILendingPool(lendingPool).getReserveData(address(WBNB)));
    ...
    ILendingPool(lendingPool).repay(address(WBNB), msg.value, rateMode, onBehalfOf);
  ...
  function borrowBNB(
    ...
    ILendingPool(lendingPool).borrow(address(WBNB), amount, interesRateMode, referralCode, msg.sender);
   ...
   * due selfdestructs or transfer bnber to pre-computated contract address before deployment.
   * @param to recipient of the transfer
   * @param amount amount to send
   */
  ...
  /**
   * @dev Only WBNB contract is allowed to transfer BNB here. Prevent other addresses to send Ether to this contract.
   */
