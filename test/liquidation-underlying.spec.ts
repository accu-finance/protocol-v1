...
describe('LendingPool: liquidationCall DAI-WETH (same decimals) with liquidator receiving underlying token', () => {
  ...
  it('reverted: liquidationCall on different collateral asset than that user2 deposited', async () => {
    ...
    const debtToCover = await convertToCurrencyDecimals(asset.DAI, DAI_NATIVE);
    await expect(
      liquidator.lendingPool.liquidationCall(asset.DAI.address, asset.DAI.address, user2.address, debtToCover, false)
    ).to.be.revertedWith(LPCM_COLLATERAL_CANNOT_BE_LIQUIDATED);
  });

  it('liquidationCall and rewards paid to liquidator in underlying token', async () => {
    ...
    //WETH
    expect(reserveDataWETHAfter.availableLiquidity).to.be.lt(reserveDataWETHBefore.availableLiquidity);
    expect(userDataWETHAfter.usageAsCollateralEnabled).to.be.true;
    expect(liquidatorDataWETHAfter.walletBalance).to.be.gt(liquidatorDataWETHBefore.walletBalance);
  });
});

describe('LendingPool: liquidationCall USDC-BTC (different decimals) with liquidator receiving underlying token', () => {
  const fixture = {} as Fixture;
  const dashboardEnabled = false;
  ...
  it('user1 deposits USDC, user2 deposits BTC as collateral and borrows up to 0.95 Health Factor', async () => {
    ...
    //need to verer to debt decimals first
  ...
  it('reverted: liquidationCall on collateral with more than 1 Health Factor', async () => {
    ...
    await expect(
      liquidator.lendingPool.liquidationCall(asset.BTC.address, asset.USDC.address, user2.address, debtToCover, false)
    ).to.be.revertedWith(LPCM_HEALTH_FACTOR_NOT_BELOW_THRESHOLD);
  });
  ...
  it('reverted: liquidationCall on different debt asset than that user2 borrowed', async () => {
    ...
    await expect(
      liquidator.lendingPool.liquidationCall(asset.BTC.address, asset.BTC.address, user2.address, debtToCover, false)
    ).to.be.revertedWith(LPCM_SPECIFIED_CURRENCY_NOT_BORROWED_BY_USER);
  });
  ...
  it('reverted: liquidationCall on different collateral asset than that user2 deposited', async () => {
    ...
    const debtToCover = await convertToCurrencyDecimals(asset.USDC, USDC_NATIVE);
    await expect(
      liquidator.lendingPool.liquidationCall(asset.USDC.address, asset.USDC.address, user2.address, debtToCover, false)
    ).to.be.revertedWith(LPCM_COLLATERAL_CANNOT_BE_LIQUIDATED);
  });
  ...
  it('liquidationCall and rewards paid to liquidator in underlying token', async () => {
    ...
    //BTC
    expect(reserveDataWBTCAfter.availableLiquidity).to.be.lt(reserveDataWBTCBefore.availableLiquidity);
    expect(userDataWBTCAfter.usageAsCollateralEnabled).to.be.true;
    expect(liquidatorDataWBTCAfter.walletBalance).to.be.gt(liquidatorDataWBTCBefore.walletBalance);
  });
});
