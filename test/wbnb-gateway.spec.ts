...
(process.env.PROVIDER_ID === MarketProvider.BscMain ? describe : describe.skip)(
  ...
  function () {
    ...
    it('user1 deposits 5 BNB and gets 5 aWBNB', async () => {
      const {user1} = fixture;

      await depositBNB('5', user1, user1, {dashboardEnabled});
    });
    ...
    it('reverted: user1 withdraws 0 BNB', async () => {
      const {user1} = fixture;

      await withdrawBNB('0', user1, user1, {dashboardEnabled, expectedResult: {revertMessage: VL_INVALID_AMOUNT}});
    });

    it('user2 borrows BNB via delegateApprove at stable rate with deposited DAI as collateral', async () => {
      const {user2} = fixture;

      await borrowBNB('1', user2, user2, RateMode.Stable, {dashboardEnabled, timeTravel: 1 * YEAR});
    });

    it('user2 repay BNB + interest', async () => {
      const {user2} = fixture;
      await repayBNB('-1', user2, user2, RateMode.Stable, {dashboardEnabled});
    });

    it('user1 withdraws some amount of BNB', async () => {
      const {user1} = fixture;

      await withdrawBNB('2.5', user1, user1, {dashboardEnabled});
    });

    it('user1 withdraws full amount of BNB', async () => {
      const {user1} = fixture;

      await withdrawBNB('-1', user1, user1, {dashboardEnabled});
    });
  }
);
...
(process.env.PROVIDER_ID === MarketProvider.BscMain ? describe : describe.skip)(
  ...
  () => {
    ...
    it('user2 borrows BNB via delegateApprove at variable rate with deposited DAI as collateral', async () => {
      const {user2} = fixture;

      await borrowBNB('1', user2, user2, RateMode.Variable, {dashboardEnabled, timeTravel: 1 * YEAR});
    });

    it('user2 repay BNB + interest', async () => {
      const {user2} = fixture;
      await repayBNB('-1', user2, user2, RateMode.Variable, {dashboardEnabled});
    });
